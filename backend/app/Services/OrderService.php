...
class OrderService
{
    public function store(User $user, UserAddress $address, $remark, $items, UserCoupon $coupon = null)
    {
        // 开启一个数据库事务
        $order = DB::transaction(function () use ($user, $address, $remark, $items, $coupon) {
            ...
            foreach ($items as $data) {
                ...
                $price = $sku->product->exclusive_price;
            ...
            if ($coupon) {
                ...
                // 把订单金额修改为优惠后的金额
                $newTotalAmount = $coupon->getAdjustedPrice($totalAmount);
                $updateData['user_coupon_id'] = $coupon->id;
                $updateData['discount_money'] = bcsub($totalAmount,$newTotalAmount,2);
                $totalAmount = $newTotalAmount;
                // 将订单与优惠券关联
                $order->user_coupon()->associate($coupon);
    ...
    public function refundOrder(Order $order)
    {
        ...
        switch ($order->payment_method) {
            // 如果是线下付款的退款申请，那么同意退款后提示输入线下退款流水号
            case 'offline_pay':
                // 生成退款订单号
            ...
            case 'wechat':
                // 生成退款订单号
                $refundNo = Order::getAvailableRefundNo();
                $refundMoney = bcmul($order->total_amount,100);
//                $refundMoney = 1;
