...
class ActivityController extends Controller
{
    ...
    protected function grid()
    {
        ...
        $grid->model()->where('type',$activityModel::TYPE_ONE)->orderBy('created_at','desc');
        $grid->column('id','Id')->sortable();
        $grid->column('uid','用户id');
        $grid->column('cover_image','封面图')->image(100,100);
        $grid->column('title','标题');
        $grid->column('activity_time','活动时间');
        $grid->column('apply_end_time','报名截止时间');
        $grid->column('status','状态')->using($statusArr);
//        $grid->column('shelves_status','状态')->using($activityModel->getShelvesStatus());
...
//        $grid->disableCreateButton();
    ...
    public function checkRecommend(Request $request) {
        $id = $request->get('id');
        $activity = new Activity();
        $old_result = $activity->where('id',$id)->select('id','recommend')->first();
        if ($old_result) {
            $recommend = $activity::RECOMMEND_STATUS_ONE;
            if ($recommend == $old_result->recommend) {
                $recommend = $activity::RECOMMEND_STATUS_TWO;
            }
            $activity->where('id',$id)->update(['recommend'=>$recommend]);
            return response()->json(['status'=>true,'message'=>'操作成功']);
        }
        return response()->json(['message'=>'操作失败，请刷新后再试']);
    }
    ...
    protected function form()
    {
        ...
        $id = isset(request()->route()->parameters()['activity']) ? request()->route()->parameters()['activity'] : null;
        if ($id) {
...
//        $form->select('type', '类型')->options($activityModel->getType())->rules('required')->default($activityModel::TYPE_ONE);
        $form->hidden('type')->default($activityModel::TYPE_ONE);
        $form->text('title', '标题')->rules('required');
        $form->datetime('activity_time', '活动时间')->default(date('Y-m-d H:i:s'))->rules('required|date|after:apply_end_time',[
            'date' => '活动时间必须是日期格式',
            'after'   => '活动时间必须晚于报名截止时间',
        ]);
        $form->datetime('apply_end_time', '报名截止时间')->default(date('Y-m-d H:i:s'))->rules('required|date|after:tomorrow',[
            'date' => '报名截止时间必须是日期格式',
            'after'   => '报名截止时间必须在今天之后',
        ]);
        $form->textarea('content', '活动内容')->rules('required');
        ...
        $form->hasMany('specification', '活动规格', function (Form\NestedForm $form) use ($id) {
