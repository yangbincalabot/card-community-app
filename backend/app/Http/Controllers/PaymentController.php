...
class PaymentController extends Controller
{
    ...
    public function wechatNotify()
    {
        ...
        if(isset($data->attach)){
            $responseAttach = strtoupper($data->attach);
            $attach = strtoupper('discover_apply');
            if($responseAttach == $attach){
                return $this->discoverApplyWechatNotify($data);
            }
        }
        ...
        $salesCommissionsService = new SalesCommissionsService;
    ...
    public function discoverApplyWechatNotify($data)
    {

        // 找到对应的订单
        $order = ActivityApply::where('order_no', $data->out_trade_no)->first();
        // 订单不存在则告知微信支付
        if (!$order) {
            return 'fail';
        }
    ...
    public function discoverApplyWechatRefundNotify($data)
    {
        ...
        if ($data['refund_status'] === 'SUCCESS') {
            ...
            $Relevance->where('apply_id',$order->id)->update([
                'status' => $Relevance::STATUS_DELETE,
            ]);
            ...
            $extra['refund_failed_code'] = $data['refund_status'];
            $order->update([
                'refund_status' => ActivityApply::REFUND_STATUS_FAILED,
                'extra' => $extra
            ]);
        }
