...
Page({
    ...
    getUserInfo: function () {
        util.get(api.GetCreateCarteInfo, {id: this.data.id}).then(res => {
            ...
            if(carte){
                ...
                this.setData({
                    'formData.cid': bind_company_id,
                    'formData.name': carte.name,
                    'formData.company_name': carte.company_name,
                    'formData.avatar': carte.avatar,
                    'formData.phone': carte.phone,
    ...
    formSubmit: function (event) {
        ...
        util.post(api.CreateOtherCarte, this.data.formData).then((res) => {
            wx.hideLoading();
            let title, url = '';
            let is_close_current = false;
            if(this.data.id === 0){
                title = '添加成功';
                url = '/pages/card/other/index?id=' + res.data.data.id;
                is_close_current = true;
            }else{
                title = '编辑成功';
                url = '/pages/card/other/index?id=' + this.data.id;
            }
