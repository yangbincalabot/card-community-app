...
Page({
    ...
    onLoad: function (options) {
        ...
        util.get(api.BusinessAssociationShowUrl, { id: this.data.id }).then(res => {
            let association = res.data.data.association
            this.setData({
                formData: association,
                is_hidden: false,
                image_url: association.image,
                pid: association.pid,
    ...
    getAssociationList: function () {
        util.get(api.SelectAssociationUrl, { id: this.data.id }).then(res => {
            ...
            console.log(_list)
            let _currentIndex = _list.findIndex((item) => item.id == this.data.pid);
            let _currentTitle = this.data.currentTitle;
            if (_currentIndex > -1) {
                _currentTitle = _list[_currentIndex].name;
            } 
            this.setData({
                associations: _list,
    ...
    formSubmit: function (event) {
        let postData = Object.assign(this.data.formData, event.detail.value, { id: this.data.id });
        // 验证表单
        ...
        util.post(api.BusinessAssociationUpdateUrl, postData).then((res) => {
            wx.hideLoading();
            wx.showToast({
                title: '操作成功',
                icon: 'success',
